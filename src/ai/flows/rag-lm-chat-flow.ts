'use server';
/**
 * @fileOverview A dedicated chat flow for the RAG LM page that proxies requests to a specific n8n webhook.
 */
...
export async function ragLmChat(request: ChatRequest): Promise<ChatResponse> {
  ...
  try {
    ...
    console.log('RAG LM Response:', JSON.stringify(responseData, null, 2));
