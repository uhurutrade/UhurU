      ...
      <div className="w-full max-w-lg z-10">
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
          <div className="p-8 pb-4">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Contáctanos</h1>
            <p className="text-slate-400">Envíanos un mensaje y te responderemos lo antes posible.</p>
          </div>

          <form action={handleAction} className="p-8 pt-4 space-y-6">
            <div className="space-y-4">
              <div className="relative group">
                <label className="text-sm font-medium text-slate-300 mb-1.5 block">Nombre completo</label>
              ...
              <div className="relative group">
                <label className="text-sm font-medium text-slate-300 mb-1.5 block">Tu mensaje</label>
                <div className="relative">
            ...
            {status && (
              <div className={`p-4 rounded-xl flex items-start gap-3 ${status.success ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                ...
                <div className="text-sm">
                  ...
                  {(!status.message && !status.success) && <p>Error al enviar: Por favor revisa los campos.</p>}
                  {(status as any).errors && (
                    <ul className="list-disc list-inside mt-1 opacity-80">
                      {Object.entries((status as any).errors).map(([field, msgs]: any) => (
