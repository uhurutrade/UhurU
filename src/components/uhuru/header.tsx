...
const WhatsAppIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-foreground hover:text-primary" fill="none">
  ...
  <TooltipProvider>
    <div className={cn(
      ...
      isMobile ? "flex-col items-start" : "flex-row items-center"
        ...
        <a href="mailto:hello@uhurutrade.com" className={cn("text-foreground hover:text-primary font-medium whitespace-nowrap", isMobile ? "text-xs" : "text-[9px] mr-1.5")}>
...
export default function Header() {
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-zinc-100/90 dark:bg-background/95 backdrop-blur-md">
      <div className="container mx-auto flex h-9 max-w-7xl items-center justify-between px-2 md:px-4">
        <div className="flex-shrink-0 scale-[0.55] origin-left">
          <Link href="/"><Logo /></Link>
        </div>

        <div className="hidden md:flex items-center gap-1 rounded-full border bg-muted/20 p-0.5 mx-1.5">
            <Link href="/uhuru-airdrop" className="px-1.5 py-0.5 text-[8.5px] font-bold text-foreground hover:bg-background/70 hover:text-primary rounded-full transition-colors uppercase whitespace-nowrap">Airdrop</Link>
            <Link href="/uhurucoin" className="px-1.5 py-0.5 text-[8.5px] font-bold text-foreground hover:bg-background/70 hover:text-primary rounded-full transition-colors uppercase whitespace-nowrap">UhuruCoin</Link>
            <Link href="/ai-chat" className="px-1.5 py-0.5 text-[8.5px] font-bold text-foreground hover:bg-background/70 hover:text-primary rounded-full transition-colors uppercase whitespace-nowrap">AI Chat</Link>
        </div>
        ...
        <div className="flex items-center gap-1.5 md:hidden">
          <ThemeToggle className="h-6 w-6" iconClassName="h-3 w-3" />
          <Sheet>
            <SheetTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Menu className="h-3.5 w-3.5" /></Button></SheetTrigger>
